...
expected_plot = plot(us, ω; field_apply=real,seriestype = :contour,c=:balance,legend=:none,size=(600,300),title="Predicted\nEnsemble Average\n");
...
anim = @animate for i ∈ 1:nb_of_configurations
    ...
    bar_y = 0.0
    progress_plot = plot([0, 2π], [bar_y, bar_y], lw=6, color=:lightgray, label = "", grid=false,axis=false, ylims=(-.01,.01))
    progress_plot = plot!([0, 2π * progress], [bar_y, bar_y], lw=6, color=:gray, label = "")
    
    plot(sc_plot,mean_sc_plot, expected_plot, progress_plot,layout=l,size=(1500,900))
end
